...
public class RoomServiceImpl implements RoomService {
	...
	@Autowired
	RoomServiceImpl(NamedParameterJdbcTemplate jdbcTemplate) {
	...
	public List<RoomStatusCustomModal> getCurrentRoomStatus() {
		...
		searchResults = jdbcTemplate.query(QueryConstants.ROOM_STATUS1,
				new BeanPropertyRowMapper<>(RoomStatusCustomModal.class));
	...
	public List<TodayBookingCustomModal> getBookingList() {
		...
		searchResults = jdbcTemplate.query(QueryConstants.BOOKING_LIST,
				new BeanPropertyRowMapper<>(TodayBookingCustomModal.class));
		return searchResults;
	}

	@Transactional(readOnly = true)
	@Override
	public List<TodayBookingCustomModal> getBookingListByGustId(int guestId) {

		Map<String, String> queryParams = new HashMap<>();
		queryParams.put("guestId", "" + guestId);

		List<TodayBookingCustomModal> searchResults = null;

		searchResults = jdbcTemplate.query(QueryConstants.BOOKING_LIST_GUEST_ID, queryParams,
				new BeanPropertyRowMapper<>(TodayBookingCustomModal.class));
	...
	public List<TodayBookingCustomModal> getTodayBookingList() {
		...
		searchResults = jdbcTemplate.query(QueryConstants.TODAY_BOOKING_LIST,
				new BeanPropertyRowMapper<>(TodayBookingCustomModal.class));
		return searchResults;
	}
	...
	public RoomTotalStatusCustomModal getTotalRoomStatus() {
		...
		totalRoomStatus = jdbcTemplate.query(QueryConstants.TOTAL_ROOM_STATUS,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));

		totalRoomPax = jdbcTemplate.query(QueryConstants.TOTAL_ROOM_PAX,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));
		totalTodayArr = jdbcTemplate.query(QueryConstants.TODAY_ARR,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));
		totalTodayDep = jdbcTemplate.query(QueryConstants.TODAY_DEP,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));
		dayUse = jdbcTemplate.query(QueryConstants.TODAY_USE,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));
		continueRooms = jdbcTemplate.query(QueryConstants.TOTAL_CONTINUE_ROOM,
				new BeanPropertyRowMapper<>(RoomTotalStatusCustomModal.class));

		// BigDecimal occPer=(new
		// BigDecimal(totalRoomStatus.get(0).getTotalRooms()).divide(new
		// BigDecimal(100)).multiply(new
		// BigDecimal(totalRoomStatus.get(0).getTotalRooms())));

		BigDecimal occPer = (new BigDecimal(totalRoomStatus.get(0).getOccupied()).multiply(new BigDecimal(100))
				.divide(new BigDecimal(totalRoomStatus.get(0).getTotalRooms()), 2, RoundingMode.HALF_UP));
